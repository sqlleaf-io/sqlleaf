...
def test__positions_duplicate_nested_functions(holder):
    ...
    assert h.nodes_full == [
     	'function[CURRENTUSER() type=VARCHAR query_depth=0 query_width=0 statement=1 select=0 func_depth=2 func_arg=0]',
     	'function[CURRENTUSER() type=VARCHAR query_depth=0 query_width=0 statement=1 select=0 func_depth=2 func_arg=1]',
    ...
    assert h.paths == [
        ['function[CURRENTUSER()]', 'function[UPPER()]', 'function[DPIPE()]', 'column[names.name]'],
        ['function[CURRENTUSER()]', 'function[UPPER()]', 'function[DPIPE()]', 'column[names.name]']
    ]
