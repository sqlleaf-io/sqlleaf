...
def collect_queries(text: str, dialect: str, object_mapping) -> (t.List[structs.Query], t.List[exp.Expression]):
    """
...
def _process_unnamed(statement: t.Union[exp.Insert, exp.Update], dialect: str, object_mapping):
    """
...
def _process_tables(statement: exp.Create, dialect: str, object_mapping):
    """
    ...
    if statement.kind == "TABLE":
        ...
        object_mapping.add_table_mapping(
            query=query,
        ...
        object_mapping.add_sequence_mapping(query=query, dialect=dialect)
    return query


def _process_views_and_ctas(statement: exp.Create, dialect: str, object_mapping):
    """
...
def _process_functions(statement: exp.Create, dialect: str, object_mapping):
    """
    ...
    object_mapping.add_user_defined_function_mapping(query=query, dialect=dialect)
...
def _process_triggers(statement: exp.Create, dialect: str, object_mapping):
    """
    Process a "CREATE TRIGGER" statement.
    """
    query = structs.TriggerQuery(statement, dialect)
    object_mapping.add_trigger_mapping(query=query, dialect=dialect)
    return query


def _process_stored_procedures(statement: exp.Create, dialect: str, object_mapping):
    """
    Process a "CREATE PROCEDURE" statement.
    """
    query = structs.ProcedureQuery(statement=statement, dialect=dialect)
    object_mapping.add_stored_procedure_mapping(query=query, dialect=dialect)
